...
def tr_manual(m):
    ...
    if dim == 2:
        index = m
        temp = beta(index[0], x)*beta(index[1], y)*z - beta(index[0]-1, x)*beta(index[1]+1, y) - beta(index[0]+1, x)*beta(index[1]-1, y)
        return expand(temp)

    result = 0
    fprint("start idx_gen, for compute trace of", m)
    for idx in idx_generator(dim, 2):
            summand = ((-1)**sum(idx)) * large_b(idx-ones+m,x,y) * tr_for_idx(dim, idx)
            result += summand
